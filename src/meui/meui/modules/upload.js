    ...
    Class.prototype.config = {
        ...
        processfail: function (e, data) {
            ...
            if (data.files.error && currentFile.error) {
                ...
                if(NBT && NBT.dialog && NBT.dialog.nbtAlert){
                    NBT.dialog.nbtAlert(function(){
                        return;
                    },currentFile.error,"错误提示");
                }else {
            ...
            $(window).on("message onmessage", function(e) {
                ...
                if (msg) {
                    ...
                    for (var i=0;i<that.options.length;i++){
                        var op = that.options[i];
                        if(op.success && jsonObject && jsonObject.messageId && jsonObject.messageId==op.formData.messageId){
                            op.success(jsonObject)
                            break;
                        }
                    }
                }
